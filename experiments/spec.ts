...
export type TranscriptEntry =
  { kind: "read"; channel: string[] } | { kind: "write"; channel: string[] };
...
function translateOuterToInner(
  chan: string[],
  mapping: Array<[string[], string[]]>,
): string[] | null {
...
function translateInnerToOuter(
  chan: string[],
  mapping: Array<[string[], string[]]>,
): string[] {
  for (const [from, to] of mapping) {
...
function equals(a: MachineSpec, b: MachineSpec): boolean {
  ...
  switch (a.kind) {
    ...
    case "choice":
      ...
      return a.choices.every((choice, i) =>
        equals(choice, (b as typeof a).choices[i]),
      );
    case "star":
    ...
    case "rename": {
      ...
      for (let i = 0; i < a.mapping.length; i++) {
        ...
        if (
          !channelsEqual(pairA[0], pairB[0]) ||
          !channelsEqual(pairA[1], pairB[1])
        )
          return false;
      }
      return equals(a.inner, (b as typeof a).inner);
    ...
    case "indexed": {
      ...
      for (let i = 0; i < keysA.length; i++) {
        ...
        if (!equals(a.active[keysA[i]], (b as typeof a).active[keysB[i]]))
          return false;
      }
      return true;
    }
    case "trace":
      return (
        channelsEqual(a.pathA, (b as typeof a).pathA) &&
...
export function isCompleted(spec: MachineSpec): boolean {
  switch (spec.kind) {
    ...
    case "indexed": {
      ...
      if (spec.then.kind === "sequence" && spec.then.specs.length === 0)
        return true;
      return spec.activated ? isCompleted(spec.then) : false;
    }
    case "trace": {
      const closure = resolveEpsilonTransitions(
        [spec.inner],
        spec.pathA,
        spec.pathB,
      );
...
function transitionNfa(
  ...
  switch (spec.kind) {
    ...
    case "write":
      if (
        entry.kind === "write" &&
        channelsEqual(spec.channel, entry.channel)
      ) {
        return [sequence()];
      }
      return [];
    ...
    case "star": {
      return transitionNfa(spec.inner, entry).map((nextInner) =>
        sequence(nextInner, spec),
      );
    }
    case "concurrent": {
    ...
    case "complement": {
      ...
      return transitionNfa(spec.inner, reversedEntry).map((nextInner) =>
        complement(nextInner),
      );
    }
    case "rename": {
    ...
    case "indexed": {
      ...
      if (index.startsWith("*") && !spec.active[index]) {
        const starCount = Object.keys(spec.active).filter((k) =>
          k.startsWith("*"),
        ).length;
        if (starCount >= 3) {
          return [];
    ...
    case "trace": {
      if (
        getSuffix(entry.channel, spec.pathA) !== null ||
        getSuffix(entry.channel, spec.pathB) !== null
      ) {
        return [];
      }
      const closure = resolveEpsilonTransitions(
  ...
  switch (spec.kind) {
    ...
    case "rename":
      for (const t of getPossibleTransitions(spec.inner)) {
        result.push({
          kind: t.kind,
          channel: translateInnerToOuter(t.channel, spec.mapping),
        });
      }
      break;
    case "prefix":
    ...
    case "trace": {
      ...
      for (const s of closure) {
        for (const t of getPossibleTransitions(s)) {
          if (
            getSuffix(t.channel, spec.pathA) === null &&
            getSuffix(t.channel, spec.pathB) === null
          ) {
            result.push(t);
          }
        }
...
function resolveEpsilonTransitions(
  states: MachineSpec[],
  pathA: string[],
  pathB: string[],
): MachineSpec[] {
    ...
    for (const ev of possible) {
      if (ev.kind === "write") {
        ...
        if (suffixA !== null) {
          ...
          const readEvent: TranscriptEntry = {
            kind: "read",
            channel: readChannel,
          };
          ...
          if (intermediateStates.length > 0) {
            ...
            if (!matched) {
              throw new Error(
                `Blocked wired transition: ${ev.channel.join(".")} has no matching reader`,
              );
            }
          }
        }
...
type DfaRunState =
  ...
  | {
      kind: "indexed";
      index: number;
      active: Record<string, DfaRunState>;
      activated: boolean;
    };
...
function isRunStateCompleted(state: DfaRunState, context: DfaContext): boolean {
  if (state.kind === "concurrent") {
    ...
    const allSubCompleted = Object.values(state.machines).every((s) =>
      isRunStateCompleted(s, context),
    );
    if (allSubCompleted) {
      if (node.continuationIndex !== undefined) {
        return isRunStateCompleted(
          initRunState(node.continuationIndex, context),
          context,
        );
      }
      return true;
  ...
  if (state.kind === "indexed") {
    ...
    const allActiveCompleted = Object.values(state.active).every((s) =>
      isRunStateCompleted(s, context),
    );
    if (!allActiveCompleted) return false;
    if (node.continuationIndex !== undefined) {
...
function transitionRunState(
  ...
  if (state.kind === "standard") {
    ...
    const matches = node.transitions.filter(
      (t) => t.kind === entry.kind && channelsMatch(t.channel, entry.channel),
    );
  ...
  if (state.kind === "indexed") {
    ...
    if (entry.channel.length > 0) {
      ...
      if (state.active[index]) {
        const nextSubs = transitionRunState(
          state.active[index],
          strippedEntry,
          context,
        );
        ...
        let allowed = true;
        if (index.startsWith("*")) {
          const starCount = Object.keys(state.active).filter((k) =>
            k.startsWith("*"),
          ).length;
          if (starCount >= 3) {
            allowed = false;
    ...
    if (
      state.activated &&
      allActiveCompleted &&
      node.continuationIndex !== undefined
    ) {
      results.push(
        ...transitionRunState(
          initRunState(node.continuationIndex, context),
...
function getRunStateTransitions(
  state: DfaRunState,
  context: DfaContext,
): TranscriptEntry[] {
  if (state.kind === "standard") {
...
function isSubtypeCompiled(a: CompiledSpec, b: CompiledSpec): SubtypeResult {
  ...
  function check(
    ...
    if (
      statesA.length === 1 &&
      statesA[0].kind === "indexed" &&
      statesB.length === 1 &&
      statesB[0].kind === "indexed"
    ) {
      ...
      if (
        nodeA.continuationIndex !== undefined ||
        nodeB.continuationIndex !== undefined
      ) {
        const doneIndexA = compile(sequence(), a.context);
        const doneIndexB = compile(sequence(), b.context);
        const idxA =
          nodeA.continuationIndex !== undefined
            ? nodeA.continuationIndex
            : doneIndexA;
        const idxB =
          nodeB.continuationIndex !== undefined
            ? nodeB.continuationIndex
            : doneIndexB;
...
export function checkTranscript(
  ...
  let currentGlobalStates: DfaRunState[] = [
    initRunState(spec.startIndex, spec.context),
  ];
...
export function parseTranscript(text: string): TranscriptEntry[] {
  ...
  for (let line of lines) {
    ...
    if (kindChar !== "<" && kindChar !== ">") {
      throw new Error(
        `Invalid line format: must start with '<' or '>', got "${line}"`,
      );
    }
    ...
    if (channel.some((part) => part === "")) {
      throw new Error(
        `Invalid channel path: contains empty segments in "${channelPart}"`,
      );
    }
