...
- **[lang/rewrite](lang/rewrite)**: The prover.
  - [lang/rewrite/src/kernel](lang/rewrite/src/kernel): The trusted kernel — everything a proof's truth rests on, and nothing that only has to be found. The term model (`term.rs`: programs as two arity-exact operators, `;` and `*`, over a handful of leaves; the language goals are lowered from and `via` waypoints are written in), programs as literal graphs of boxes and wires (`graph.rs`), the translation from one to the other (`mod.rs`), the law table (`rules.rs`), and the proof checker (`goal.rs`). A bug here could let a false identity through; a bug anywhere else in the crate produces a step the kernel refuses or a proof that fails to check.
