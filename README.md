...
- **Independent Stack Windows**: `par { A } { B } ...` cuts the stack into one window per arm — sized by each arm's own arity, first arm deepest — runs the arms side by side, and glues the results back together. No arm can observe another's window, so the arms may be reordered or analysed one at a time. `dip N { X }` is exactly `par { X } { id N }`.
...
- **Static Arity Verification**: An arity checker runs before execution to ensure that stack push/pop operations match function signatures, avoiding runtime stack underflows. A `branch`'s two arms are held to the *same arity*, not merely the same net stack change; the compiler pads the narrower arm with `par { id K } { arm }` first, so nothing has to be spelled out. See [docs/compilation.md](docs/compilation.md).
...
| :--- | :--- | :--- |
| **Stack Ops** | `Push(V)`, `Drop`, `Pick(d)`, `Roll(d)`, `Id(n)` | Standard stack push, pop, copy/peek at depth, and rotate. `Pick` and `Roll` are the only instructions that address below the top of the stack. `Id(n)` does nothing at run time and asks for `n` values, which is how a demand nothing consumes gets written down. |
...
| **Control Flow** | `Dip(n, S)`, `Par([S, ...])`, `Branch(S1, S2)`, `Panic`, `Assert`, `AssertEqual` | Subroutine execution under a hidden region of the stack (a plain `jump` is `Dip(0, S)`), running sentences side by side on windows of the stack (`Dip(n, S)` is `Par([S, id n])`), conditional branching, and explicit panics. |
