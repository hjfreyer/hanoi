...
| :--- | :--- | :--- |
| **Stack Ops** | `Push(V)`, `Drop`, `Copy`, `Swap` | Push, discard, duplicate the top value, exchange the top two. No instruction takes a depth today. The surface language's `pick d`, `roll d` and `drop d` are spellings the compiler expands into frames around these. |
...
- **[lang/rewrite](lang/rewrite)**: The prover.
  - [lang/rewrite/src/kernel](lang/rewrite/src/kernel): The trusted kernel — everything a proof's truth rests on, and nothing that only has to be found. The term model (`term.rs`: programs as two arity-exact operators, `;` and `*`, over a handful of leaves; the language goals are lowered from and `via` waypoints are written in), programs as literal graphs of boxes and wires (`graph.rs`), the translation from one to the other (`mod.rs`), the law table (`rules.rs`), and the kernel's one judgement of a proof (`goal.rs`: `certify`, which replays a flat run of steps on a claim's left side and asks whether it landed on the right). A bug here could let a false identity through; a bug anywhere else in the crate produces a step the kernel refuses or a run that does not land.
  - [lang/rewrite/src/proof.rs](lang/rewrite/src/proof.rs): The *draft* a strategy writes — the tree of goals it carved and the steps each spent — and `flatten`, which turns the draft into the one flat run the kernel is handed, saying as steps everything the tree said as structure.
  - [lang/rewrite/src/tactic.rs](lang/rewrite/src/tactic.rs): The tactic language that drives the table, and the queries ([query.rs](lang/rewrite/src/query.rs)) it points with; [render.rs](lang/rewrite/src/render.rs) lays a stuck graph out for reading. All untrusted: every step they take goes back through the kernel.
